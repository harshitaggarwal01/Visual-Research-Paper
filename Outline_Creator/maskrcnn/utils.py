...
def extract_elements(image, box, label, ROI_number, path):
    ROI = image[box[1]:box[3]+1, box[0]:box[2]+1]
    custom_config = r'--oem 3 --psm 6'
    print(pytess.image_to_string(ROI, config=custom_config))
    cv2.imwrite(path+"/"+label+'_{}.png'.format(ROI_number), ROI)
    print("Label:")
    print(label+str(ROI_number))